...
import { ThemeColor, LoraSelection } from '../types';
...
interface LoraControlProps {
  ...
  // Unified Callbacks
  onUpdate: (id: string, updates: Partial<LoraSelection>) => void;
  onDelete?: (id: string) => void;

  theme: ThemeColor;
}

const LoraControl: React.FC<LoraControlProps> = React.memo(({
  id,
  label,
  enabled = true,
  strength,
  ...
  return (
    <div className={`p-4 rounded-lg border transition-all group ${enabled
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex flex-col flex-1 mr-4 min-w-0">
          ...
          {!readOnly && availableLoras.length > 0 ? (
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {!readOnly && (
            <button onClick={() => onUpdate(id, { enabled: !enabled })} className={`text-${theme}-500 dark:text-${theme}-400 hover:text-${theme}-600 dark:hover:text-${theme}-300 transition-colors`}>
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          step="0.05"
          value={strength}
          onChange={(e) => onUpdate(id, { strength: parseFloat(e.target.value) })}
