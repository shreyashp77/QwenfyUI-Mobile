...
const ServerImageSelector: React.FC<ServerImageSelectorProps> = ({ serverAddress, images, onSelect, onClose, theme }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-gray-900 w-full max-w-2xl h-[80vh] rounded-xl border border-gray-700 shadow-2xl flex flex-col">
        ...
        <div className="p-3 bg-gray-900 border-b border-gray-800">
            <div className="relative">
                ...
                <input 
                    ...
                    className={`w-full bg-gray-800 text-white rounded-lg pl-9 pr-3 py-2 text-sm border border-gray-700 focus:border-${theme}-500 outline-none`}
                />
            </div>
        </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {filteredImages.map((filename) => (
                        <button 
                            ...
                            className={`group relative aspect-square rounded-lg overflow-hidden border border-gray-700 bg-black hover:border-${theme}-500 transition-all text-left`}
                        >
                            <img 
                                loading="lazy"
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-black/70 p-1.5 backdrop-blur-sm">
                                <p className="text-[10px] text-gray-200 truncate font-mono">{filename}</p>
                            </div>
                        </button>
                    ))}
                </div>
